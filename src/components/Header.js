...
const Header = (props) => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        if (!selectedMovieDetails) navigate('/browse')
      } else {
        // dispatch(setGptSearchView(false));
        dispatch(removeUser())
        navigate('/')
  ...
  return (
    <div
      className={`w-screen md:py-0 md:px-6 bg-black bg-opacity-[${props.opacity}%]  z-10 flex flex-col md:flex-row justify-between`}
    >
      <img
