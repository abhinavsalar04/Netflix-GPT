...
const MovieDetailsMainContainer = () => {
  const selectedMovieDetails = useSelector(
    (store) => store.movies.selectedMovieDetails,
  )
  const selectedMovieCredits = useSelector(
    (store) => store.movies.selectedMovieCredits,
  )
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row pt-12 pb-8 md:pt-20 justify-center bg-inherit  text-white md:px-[10%]">
        <div className="md:pr-8 lg:pr-8 flex-shrink-0 px-2">
          <img
            className="w-screen md:w-[350px] lg:[350px] rounded-lg"
        ...
        <div className="md:max-w-[750px] my-8 ml-6 md:py-0 md:my-0">
          ...
          <div className="flex items-center my-4">
            ...
            >
              <FiPlayCircle
                style={{ fontSize: '80px' }}
                className="transition-all ease-out 300 hover:text-red-700"
              />
            </div>
            <div
              onClick={() => handleWatchTrailer(selectedMovieDetails.id)}
          ...
          <div>
            <div className="flex my-4 ">
              <div className="font-bold flex mr-[8px] flex-wrap">
                ...
                <span className="font-normal mr-[8px] leading-[24px] text-gray-500">
                  {status}
                </span>
              </div>
            ...
            <hr
              className="lg:w-[108%] md:w-[120%] w-[99%]"
              style={{ borderTop: '1px solid #3e3f40' }}
            ></hr>
            <div className="flex my-4">
        ...
        <VideoPopup showTrailer={showTrailer} setShowTrailer={setShowTrailer} />
      </div>
    </>
  )
}
