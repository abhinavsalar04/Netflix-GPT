...
const useHomePageMovieTrailer = (movieId) => {
  const homePageTrailerVideo = useSelector(
    (store) => store.movies.homePageTrailerVideo,
  )
  ...
  const getMovieVideos = async () => {
    ...
    const filterVideos = jsonData.results.filter(
      (video) => video?.type === 'Trailer',
    )

    const trailer =
      filterVideos.length !== 0 ? filterVideos[0] : jsonData.results[0]
